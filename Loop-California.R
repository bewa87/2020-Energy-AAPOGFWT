...
######################################
# Step 3: Loop for NOAA Station Data #
######################################

# DEFINE PATH OF SORTED FULL WIND SPEED STATION DATA AND ONE SORTED FILE OF STATION META DATA BY STATION ID!

OFiles     <- list.files(path="YOUR PATH OF FULL WIND SPEED DATA", pattern="*.txt", full.names=TRUE, recursive=FALSE)
Stat_Data  <- read.table("YOUR PATH OF SORTED STATION DATA (CSV)",header=TRUE,sep=";")
...
GoalMatrix  <- array(0, dim=c(length+1,20,1))
GoalMatrix2 <- array(0, dim=c(length+1,11,1))

# Step 4.1: Create First Row with Headings

GoalMatrix[1,1,1]   <- "StationID"
GoalMatrix[1,2,1]   <- "Longitude"
GoalMatrix[1,3,1]   <- "Latitude"
GoalMatrix[1,4,1]   <- "StationHeight"
GoalMatrix[1,5,1]   <- "ReadingHeight"
GoalMatrix[1,6,1]   <- "NumberOfData"
GoalMatrix[1,7,1]   <- "MeanValue"
GoalMatrix[1,8,1]   <- "StandardDeviation"
GoalMatrix[1,9,1]   <- "MinVelocity"
GoalMatrix[1,10,1]  <- "MaxVelocity"
GoalMatrix[1,11,1]  <- "Weibull_k"
GoalMatrix[1,12,1]  <- "Weibull_A"
GoalMatrix[1,13,1]  <- "Semi-empirical_Pow"
GoalMatrix[1,14,1]  <- "Estimated_Pow_Weibull"
GoalMatrix[1,15,1]  <- "Error_Weibull"
GoalMatrix[1,16,1]  <- "Difference_Weibull"
GoalMatrix[1,17,1]  <- "Estimated_Pow_Kappa"
GoalMatrix[1,18,1]  <- "Difference_Kappa"
GoalMatrix[1,19,1]  <- "Estimated_Power_Wakeby"
GoalMatrix[1,20,1]  <- "Difference_Wakeby"
...
for(j in 1:length){
  ...
  A                    <- read.table(OFiles[j], header=TRUE, sep=";")
  GoalMatrix[j+1,1,1]  <- Stat_Data$STATION_ID[j]         #Station ID
  GoalMatrix[j+1,2,1]  <- Stat_Data$LONGITUDE[j]          #Longitude
  GoalMatrix[j+1,3,1]  <- Stat_Data$LATITUDE[j]           #Latitude
  GoalMatrix[j+1,4,1]  <- Stat_Data$ELEVATION[j]          #Height of Actual Station
  GoalMatrix[j+1,5,1]  <- 10                              #Reading Height of Actual Station (Assumption: 10 meters)
  ...
  GoalMatrix[j+1,6,1]  <- length(C)
  GoalMatrix[j+1,7,1]  <- mean(C)
  GoalMatrix[j+1,8,1]  <- sd(C)
  GoalMatrix[j+1,9,1]  <- min(C)
  GoalMatrix[j+1,10,1] <- max(C)
  ...
  GoalMatrix[j+1,11,1] <- wei_k_fitdist
  GoalMatrix[j+1,12,1] <- wei_A_fitdist
  GoalMatrix[j+1,13,1] <- sum(Powercurve(C))*24*365/(as.numeric(GoalMatrix[j+1,6,1])*1000000)
  ...
  GoalMatrix[j+1,14,1]   <- P_annual_weibull
  GoalMatrix[j+1,15,1]   <- Delta_P_annual_weibull
  GoalMatrix[j+1,16,1]   <- abs(as.numeric(GoalMatrix[j+1,13,1])-as.numeric(GoalMatrix[j+1,14,1]))
  ...
  GoalMatrix[j+1,17,1]   <- P_annual_kappa
  GoalMatrix[j+1,18,1]   <- abs(as.numeric(GoalMatrix[j+1,13,1])-as.numeric(GoalMatrix[j+1,17,1]))
  ...
  GoalMatrix[j+1,19,1]   <- P_annual_wakeby
  GoalMatrix[j+1,20,1]   <- abs(as.numeric(GoalMatrix[j+1,13,1])-as.numeric(GoalMatrix[j+1,19,1]))
  
  # Interesting Results in other Matrix
  
  GoalMatrix2[j+1,1,1]  <- Stat_Data$STATION_ID[j]
  GoalMatrix2[j+1,2,1]  <- GoalMatrix[j+1,11,1]
  GoalMatrix2[j+1,3,1]  <- GoalMatrix[j+1,12,1]
  GoalMatrix2[j+1,4,1]  <- GoalMatrix[j+1,13,1]
  GoalMatrix2[j+1,5,1]  <- GoalMatrix[j+1,14,1]
  GoalMatrix2[j+1,6,1]  <- GoalMatrix[j+1,15,1]
  GoalMatrix2[j+1,7,1]  <- GoalMatrix[j+1,16,1]
  GoalMatrix2[j+1,8,1]  <- GoalMatrix[j+1,17,1]
  GoalMatrix2[j+1,9,1]  <- GoalMatrix[j+1,18,1]
  GoalMatrix2[j+1,10,1] <- GoalMatrix[j+1,19,1]
  GoalMatrix2[j+1,11,1] <- GoalMatrix[j+1,20,1]
